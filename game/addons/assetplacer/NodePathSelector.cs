...
public partial class NodePathSelector : Node
{
    ...
    public void SetNodePath(Node node)
    {
        ...
        if (node != null && IsValidType(node))
        {
            ...
            nodeIcon = _editorInterface.GetBaseControl().GetThemeIcon(nodeName, "EditorIcons");
            if (nodeIcon == _editorInterface.GetBaseControl().GetThemeIcon("notavalidiconname", "EditorIcons")) // File Broken icon
            {
                nodeIcon = _editorInterface.GetBaseControl().GetThemeIcon("Node", "EditorIcons");
            }
            _nodePathSelectorUi.SetNode(Node, nodeIcon);
        }
        else
        {
            _nodePath = NullPath;
    ...
    private void UpdateNodePath(string path)
    {
        if (_sceneRoot == null)
        {
            ...
            SetNodePath(null);
            _nodePath = null; // this is to prevent the path==NullPath, thus when a root will be added, it will be set as the node.
            return;
        }
