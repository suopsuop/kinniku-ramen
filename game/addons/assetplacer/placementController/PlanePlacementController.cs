...
public partial class PlanePlacementController : AssetPlacementController
{
	...
	private string ProcessConfiguringPlane(Node sceneRoot, Viewport viewport, bool rmbPressed)
	{
		...
		var snap = InputManager.ctrlPressed != snapping.TranslateSnappingActive();
		...
		if (snap)
		{
			step = InputManager.shiftPressed ? snapping.TranslateShiftSnapStep: snapping.TranslateSnapStep;
	...
	public override bool ProcessInput(InputManager.InputEventType inputEventType, Vector2 viewportMousePos)
	{
		if (_state == PlaneControllerState.ConfiguringPlane)
			...
			{
				case InputManager.InputEventType.Placement 
					or InputManager.InputEventType.Confirm:
					HidePlaneMovingGizmo();
					_placementPlaneGizmo?.ShowTemporarily();
					_state = PlaneControllerState.PlacingAssets;
					return true;
				case InputManager.InputEventType.Cancel:
					CancelConfiguringPlane();
					return true;
				default:
					return false;
