...
public partial class SurfacePlacementController : AssetPlacementController
{
    ...
    protected override void OnActiveChanged()
    {
        ...
        if (Active && editorInterface.IsPluginEnabled(ContextlessPlugin.PluginTerrain3D) && _root != null && !GetType().IsSubclassOf(typeof(SurfacePlacementController)))
        {
            if(CheckForTerrain3DWithoutCollisionsRec(_root)) GD.PushWarning("To place on Terrain3D in 'Surface' placement mode, debug collisions need to be enabled");
        }
    ...
    public override bool ProcessInput(InputManager.InputEventType inputEventType, Vector2 viewportMousePos)
    {
        return false;
    }
